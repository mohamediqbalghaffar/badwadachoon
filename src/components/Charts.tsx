...
import { PieChart as PieChartIcon, Building2, TrendingUp } from "lucide-react";
...
export const IncomingCharts = () => {
  ...
  const senderData = useMemo(() => {
    const counts: Record<string, number> = {};
    filteredIncomingData.forEach((d) => {
      const sender = d.sender || "نەزانراو";
    ...
    return Object.entries(counts)
      ...
      .sort((a, b) => b.count - a.count)
      .slice(0, 7); // Top 7 to match screenshot
  }, [filteredIncomingData]);
  ...
  if (filteredIncomingData.length === 0) return null;

  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 mt-2">
      {/* Letter Type Donut Chart */}
      <div className="glass glass-card p-6 relative overflow-hidden group col-span-1 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
        ...
        <div className="flex flex-col-reverse md:flex-row items-center justify-between gap-6" dir="rtl">
          {/* Legend on the Left */}
          <div className="w-full md:w-1/2 flex flex-col gap-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
            {typeData.map((entry, index) => (
              <div 
                key={index} 
                className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-slate-100/50 dark:bg-slate-800/50 border border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer"
                onClick={() => {
                  setFilters((prev: any) => {
                    const isSelected = prev.letterType?.includes(entry.name);
                    return { ...prev, letterType: isSelected ? [] : [entry.name] };
                  });
                  document.getElementById('data-table-section')?.scrollIntoView({ behavior: 'smooth' });
                }}
              >
                <div className="flex items-center gap-3">
                  <span className="font-bold text-slate-700 dark:text-slate-300 text-sm">{entry.name}</span>
          ...
          <div className="w-full md:w-1/2 h-64 relative flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip
                  cursor={{ fill: 'rgba(241, 245, 249, 0.2)' }}
                  contentStyle={{ borderRadius: '1rem', border: 'none', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1)' }}
                  formatter={(value: any, name: any) => [`${value} نامە`, name]}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="glass glass-card p-6 relative overflow-hidden group col-span-1 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
        ...
        <div className="flex-1 min-h-[300px]" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={senderData} margin={{ top: 25, right: 10, left: -20, bottom: 20 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#cbd5e1" opacity={0.3} />
              <XAxis dataKey="abbr" tick={{ fontSize: 12, fill: '#64748b', fontWeight: 'bold' }} axisLine={false} tickLine={false} dy={10} />
              <YAxis tick={{ fontSize: 12, fill: '#64748b' }} axisLine={false} tickLine={false} />
              <Tooltip
                cursor={{ fill: 'rgba(241, 245, 249, 0.2)' }}
                contentStyle={{ borderRadius: '1rem', border: 'none', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1)' }}
                formatter={(value: any, name: any, props: any) => [value, props.payload.name]}
                labelFormatter={(abbr) => {
                  const entry = senderData.find(d => d.abbr === abbr);
                  return entry ? entry.name : abbr;
                }}
              />
              <Bar 
      ...
      {/* Timeline Area Chart */}
      <div className="glass glass-card p-6 flex flex-col h-[400px] lg:col-span-2 relative overflow-hidden group border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
        <div className="flex justify-between items-center mb-6" dir="rtl">
          <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
        ...
        <div className="flex-1 min-h-0" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <defs>
                <linearGradient id="colorTimelineIncoming" x1="0" y1="0" x2="0" y2="1">
              ...
              <Area 
                ...
                strokeWidth={3} 
                fillOpacity={1} 
                fill="url(#colorTimelineIncoming)" 
