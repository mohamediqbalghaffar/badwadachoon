      ...
      <div className="relative w-full flex-1 flex items-center justify-center min-h-[500px]">
            ...
            <motion.div variants={itemVariants} className="w-full h-[380px] glass rounded-3xl p-6 border-t border-t-white/30 border-l border-l-white/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)] backdrop-blur-3xl" dir="ltr">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={timelineData} margin={{ top: 20, right: 20, left: -20, bottom: 0 }} onClick={(e: any) => { if (e?.activePayload?.length > 0) { const m = e.activePayload[0].payload.date; setDrillDown({ title: "نامەکانی مانگی " + m, data: baseFilteredData.filter((d: any) => d.sentDate && d.sentDate.startsWith(m)), viewType: "received" }); } }} style={{ cursor: "pointer" }}>
                  ...
                  <Tooltip itemStyle={{ color: '#fff' }} contentStyle={{ borderRadius: '1rem', border: 'none', background: 'rgba(15, 23, 42, 0.9)', color: '#fff' }} />
        ...
        {activeView === 'sent' && activeSlide === 0 && (
          <motion.div key="sent-slide-0" variants={containerVariants} initial="hidden" animate="visible" exit="exit" className="w-full max-w-5xl flex flex-col">
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 gap-6 mt-4">
              <div onClick={() => setDrillDown({ title: 'سەرجەم نێردراوەکان', data: baseFilteredSentData, viewType: 'sent' })} className="glass p-12 rounded-3xl flex flex-col items-center justify-center text-center border-t border-t-white/30 border-l border-l-white/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)] backdrop-blur-3xl relative overflow-hidden group hover:scale-[1.02] transition-transform cursor-pointer">
        ...
        {activeView === 'incoming' && activeSlide === 0 && (
          <motion.div key="incoming-slide-0" variants={containerVariants} initial="hidden" animate="visible" exit="exit" className="w-full max-w-5xl flex flex-col">
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 gap-6 mt-4">
              <div onClick={() => setDrillDown({ title: 'سەرجەم هاتووەکان', data: baseFilteredIncomingData, viewType: 'incoming' })} className="glass p-12 rounded-3xl flex flex-col items-center justify-center text-center border-t border-t-white/30 border-l border-l-white/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)] backdrop-blur-3xl relative overflow-hidden group hover:scale-[1.02] transition-transform cursor-pointer">
                  <div className="absolute inset-0 bg-teal-500/5 group-hover:bg-teal-500/10 transition-colors" />
                  <Send className="text-teal-500 mb-6 relative z-10" size={56} />
