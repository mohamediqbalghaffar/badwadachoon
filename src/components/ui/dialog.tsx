...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-black/80 backdrop-blur-xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
  />
...
const DialogContent = React.forwardRef<
  ...
  DialogContentProps
>(({ className, children, toggleIsDone, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay />
    <DialogPrimitive.Content
      ref={ref}
      onCloseAutoFocus={(e) => {
        // Find any element with a specific data attribute to trigger a save
        const activeTextarea = (e.target as HTMLElement).querySelector<HTMLElement>('[data-save-on-close="true"]');
        if (activeTextarea) {
          activeTextarea.blur(); // Trigger the blur event which should save
        }
      }}
      className={cn(
        ...
        "bg-white dark:bg-[#1a1a2e] text-gray-900 dark:text-gray-100 border-gray-200 dark:border-gray-800",
        "rtl:text-right",
        className
      )}
      {...props}
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-reverse",
      className
    )}
    {...props}
  />
