...
export const KPICards = () => {
  const { baseFilteredData, sentData, incomingData, setFilters, clearFilters, activeView } = useData();

  const activeData = activeView === 'received' ? baseFilteredData : activeView === 'sent' ? sentData : incomingData;

  const totalLetters = activeData.length;
  
  const pendingLetters = activeView === 'received' ? activeData.filter((item: any) => !item.responseDate).length : 0;
  
  const completedLetters = activeView === 'received' ? activeData.filter((item: any) => item.processingTime !== null && item.processingTime !== undefined) : [];
  const avgProcessingTime =
    completedLetters.length > 0
      ? completedLetters.reduce((acc: number, curr: any) => acc + (curr.processingTime ?? 0), 0) / completedLetters.length
      : 0;
      ...
      {activeView === 'received' && (
        ...
        >
          <div>
            ...
            <h3 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-emerald-500 to-teal-500">
              {avgProcessingTime.toFixed(1)} <span className="text-sm text-emerald-600/70 font-medium ml-1">ڕۆژ</span>
            </h3>
          </div>
