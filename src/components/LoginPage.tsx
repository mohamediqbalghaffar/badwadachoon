...
export const LoginPage = () => {
  ...
  const handleStaffSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        setError("ناوی بەکارهێنەر یان تێپەڕوشە هەڵەیە.");
        setShake(true);
        setTimeout(() => setShake(false), 500);
      } else if (res?.ok) {
        await updateSession();
  ...
  return (
    <div className="relative z-10 w-full min-h-[135vh] flex flex-col items-center justify-center p-4 py-12">
      <div className="absolute top-8 md:top-12 animate-in fade-in slide-in-from-top-8 duration-1000">
        ...
        {activeTab === "local" ? (
          ...
          <div className="space-y-5">
            <form onSubmit={handleStaffSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 ml-1">ناوی بەکارهێنەر</label>
                <div className="relative group">
              ...
              <button
                ...
                className="w-full mt-2 flex items-center justify-center gap-2 py-3.5 rounded-2xl text-white font-semibold transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 shadow-blue-500/25 disabled:opacity-50"
              >
                <span>چوونەژوورەوەی ستاف</span>
                <ArrowRight size={18} className="rotate-180" />
              </button>
            </form>
            ...
            <div className="relative flex items-center py-2">
              ...
              <span className="flex-shrink-0 mx-4 text-slate-400 dark:text-slate-500 text-xs font-medium">یان لە ڕێگەی</span>
              <div className="flex-grow border-t border-slate-200 dark:border-slate-700/50"></div>
            </div>
            ...
            <div className="space-y-3">
              <button
                ...
                disabled={isSubmitting}
                className="w-full flex items-center justify-center gap-3 py-3 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-sm transition-all shadow-sm hover:shadow-md hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50"
              >
